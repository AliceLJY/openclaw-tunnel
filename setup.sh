...
PLUGIN_RUNTIME_CONFIG="${SCRIPT_DIR}/.runtime/openclaw-plugin-config.json"
...
read -rsp "  Callback bot token (input hidden): " CALLBACK_BOT_TOKEN
echo ""
...
# ── Write private runtime config ───────────────────

echo ""
echo "  🔐 Generating private runtime credentials and configuration..."
export PORT CALLBACK_BOT_TOKEN CALLBACK_CHANNEL CLAUDE_PATH CODEX_PATH GEMINI_PATH
node "${SCRIPT_DIR}/scripts/write-runtime-config.mjs"
unset CALLBACK_BOT_TOKEN

echo "  📄 Written with mode 0600: .env"
echo "  📄 Written with mode 0600: .runtime/runner.env"
echo "  📄 Written with mode 0600: .runtime/openclaw-plugin-config.json"
echo "  🔒 A 256-bit worker credential was generated and was not displayed."
echo "  🔒 The callback bot token stays in task-api .env and is not copied to runner/plugin config."
...
if [[ "$(uname)" == "Darwin" ]]; then
  ...
  if [[ "$INSTALL_LA" =~ ^[Yy] ]]; then
    ...
    echo "  Skipped. Run manually: cd runner && node --env-file=../.runtime/runner.env worker.js"
  fi
else
  echo "  ℹ️  Linux detected. Run the runner manually:"
  echo "     cd runner && node --env-file=../.runtime/runner.env worker.js"
fi
...
echo ""
echo "  4. Merge this private snippet into your existing OpenClaw config:"
echo "     ${PLUGIN_RUNTIME_CONFIG}"
echo "     Keep the destination config private (mode 0600)."
echo ""
echo "  5. Try it out:"
echo "     /cc hello"
echo ""
